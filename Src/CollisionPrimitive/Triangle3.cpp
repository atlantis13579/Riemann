...
bool Triangle3::RayIntersectTriangle(const Vector3& Origin, const Vector3& Direction, const Vector3& A, const Vector3& B, const Vector3& C, float* t)
{
...
bool Triangle3::IntersectRay(const Vector3& Origin, const Vector3& Direction, float* t) const
{
	return RayIntersectTriangle(Origin, Direction, v0, v1, v2, t);
}

bool Triangle3::IntersectSegment(const Vector3& P0, const Vector3& P1) const
{
...
static int IntersectSegment_Coplanar(const Triangle3& triangle, const Plane3& plane, const Vector3& end0, const Vector3& end1, Vector3* OutPts)
{
...
// Assume end0 and end1 on opposite side of the plane(triangle), otherwise the result is incorrect
static bool IntersectSegment_Coplanar(const Triangle3& triangle, const Plane3& plane, const Vector3& end0, const Vector3& end1, Triangle3::Triangle3IntersectionResult& Result)
{
	Result.Quantity = IntersectSegment_Coplanar(triangle, plane, end0, end1, Result.Points);
...
bool Triangle3::IntersectSegmentEx(const Vector3& P0, const Vector3& P1, Segment3IntersectionResult& Result) const
{
	Plane3 plane(v0, v1, v2);
	int sign0 = plane.CalculateRelationsToPoint(P0);
	int sign1 = plane.CalculateRelationsToPoint(P1);
	...
	Result.Quantity = IntersectSegment_Coplanar(*this, plane, P0, P1, Result.Points);
...
// -----
bool Triangle3::IntersectAABB(const Vector3& A, const Vector3& B, const Vector3& C, const Vector3& Bmin, const Vector3& Bmax)
{
...
bool Triangle3::IntersectAABB(const Vector3& Bmin, const Vector3& Bmax) const
{
	return IntersectAABB(v0, v1, v2, Bmin, Bmax);
}

bool Triangle3::IntersectSphere(const Vector3& Center, float Radius) const
{
...
bool Triangle3::IntersectCapsule(const Vector3& X0, const Vector3& X1, float Radius) const
{
	Capsule3 capsule(X0, X1, Radius);
	return capsule.IntersectTriangle(v0, v1, v2);
}

bool Triangle3::IntersectTriangle(const Vector3& A, const Vector3& B, const Vector3& C) const
{
	const float Tolerance = 1e-6f;
...
static void CalculateRelationsToPlane(const Triangle3& triangle, const Plane3& plane, float distance[3], int sign[3], int& positive, int& negative, int& zero)
{
	const float Tolerance = 1e-6f;
...
bool Triangle3::IntersectTriangle(const Triangle3& triangle1, Triangle3IntersectionResult& Result) const
{
	const Triangle3& triangle0 = *this;
	...
	int sign1[3];
	float dist1[3];
	CalculateRelationsToPlane(triangle1, Plane0, dist1, sign1, pos1, neg1, zero1);
		...
		if (zero1 == 2)
		{
			for (int i = 0; i < 3; ++i)
			{
				if (sign1[i] != 0)
				{
					const int iM = (i + 2) % 3;
					const int iP = (i + 1) % 3;
					return IntersectSegment_Coplanar(triangle0, Plane0, triangle1[iM], triangle1[iP], Result);
				}
			}
		}
		else
	...
	if (zero1 == 0)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			if (sign1[i] == iSign)
			{
				...
				return IntersectSegment_Coplanar(triangle0, Plane0, intr0, intr1, Result);
			}
		}
	}
	...
	for (int i = 0; i < 3; ++i)
	{
		if (sign1[i] == 0)
		{
			...
			return IntersectSegment_Coplanar(triangle0, Plane0, triangle1[i], intr0, Result);
		}
	}
...
// static
float Triangle3::SqrDistancePointToTriangle(const Vector3& Point, const Vector3& A, const Vector3& B, const Vector3& C)
{
...
bool Triangle3::ContainsPoint(const Vector3& point) const
{
