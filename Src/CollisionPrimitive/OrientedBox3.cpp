...
namespace Riemann
{
	...
	Box3 OrientedBox3::ComputeBoundingVolume(const Vector3& Center, const Vector3& Extent, const Matrix3& Rot)
	{
		Box3 box;
		box.SetEmpty();
	...
	Box3 OrientedBox3::CalculateBoundingVolume() const
	{
		return OrientedBox3::ComputeBoundingVolume(Center, Extent, Rotation);
	}

	void OrientedBox3::ProjectAxis(const Vector3& Axis, float* t0, float* t1) const
	{
