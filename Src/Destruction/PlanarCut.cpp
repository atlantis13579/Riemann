...
namespace Riemann
{
	...
	bool PlanarCellMeshes::Build(const PlanarCells& Cells, const Box3& DomainBounds, const PlanarCutOptions& Options)
	{
		...
		if (Cells.IsInfinitePlane())
		{
			...
			if (!PlanarCut::BuildHalfSpaceMesh(DomainMesh, Plane.GetOrigin(), Plane.Normal, false, Options, Meshes[0]) ||
				!PlanarCut::BuildHalfSpaceMesh(DomainMesh, Plane.GetOrigin(), Plane.Normal, true, Options, Meshes[1]))
			{
				Clear();
				return false;
			}
	...
	bool PlanarCut::BuildHalfSpaceMesh(
		const DynamicMesh& SourceMesh,
	...
	bool PlanarCut::BuildCellMeshes(
	...
	bool PlanarCut::CutWithPlanarCells(
		const DynamicMesh& SourceMesh,
		const PlanarCells& Cells,
	...
	bool PlanarCut::CreateCuttingSurfacePreview(
		const PlanarCells& Cells,
		const Box3& DomainBounds,
