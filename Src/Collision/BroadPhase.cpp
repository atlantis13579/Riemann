...
private:
	...
	{
		const Box3d& box = m_pObjects->at(bv_i)->GetBoundingBoxWorld();
		float* p = (float*)&box;
	...
	{
		const Box3d& box1 = m_pObjects->at(bv_i)->GetBoundingBoxWorld();
		const Box3d& box2 = m_pObjects->at(bv_j)->GetBoundingBoxWorld();
