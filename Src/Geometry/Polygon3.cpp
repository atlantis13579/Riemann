...
#define POLY_STACK_SIZE		64
	...
	Vector3 ClosestPtTetrahedronToPoint(const Vector3& p, const Vector3& a, const Vector3& b, const Vector3& c, const Vector3& d)
	{
		...
		{
			const Vector3 q = Triangle3::ClosestPointOnTriangleToPoint(p, a, b, c);
		...
		{
			const Vector3 q = Triangle3::ClosestPointOnTriangleToPoint(p, a, c, d);
			const float dist = DotProduct(q - p, q - p);
			if (dist < min_dist)
			{
				min_dist = dist;
		...
		{
			const Vector3 q = Triangle3::ClosestPointOnTriangleToPoint(p, a, d, b);
			const float dist = DotProduct(q - p, q - p);
			if (dist < min_dist)
			{
				min_dist = dist;
		...
		{
			const Vector3 q = Triangle3::ClosestPointOnTriangleToPoint(p, b, d, c);
			const float dist = DotProduct(q - p, q - p);
			if (dist < min_dist)
			{
				min_dist = dist;
